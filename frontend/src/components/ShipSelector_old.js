...
import apiService from '../services/apiService';\n\n// Custom dropdown component that supports mixed fonts\nconst CustomDropdown = ({ value, options, onChange, placeholder = \"-- No Upgrade --\", style = {} }) => {\n  const [isOpen, setIsOpen] = React.useState(false);\n  const [selectedOption, setSelectedOption] = React.useState(null);\n  const dropdownRef = React.useRef(null);\n\n  // Find selected option\n  React.useEffect(() => {\n    const selected = options.find(opt => opt.value === value);\n    setSelectedOption(selected);\n  }, [value, options]);\n\n  // Close dropdown when clicking outside\n  React.useEffect(() => {\n    const handleClickOutside = (event) => {\n      if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {\n        setIsOpen(false);\n      }\n    };\n    document.addEventListener('mousedown', handleClickOutside);\n    return () => document.removeEventListener('mousedown', handleClickOutside);\n  }, []);\n\n  const handleSelect = (option) => {\n    onChange(option.value);\n    setIsOpen(false);\n  };\n\n  return (\n    <div ref={dropdownRef} style={{ position: 'relative', ...style }}>\n      <div\n        onClick={() => setIsOpen(!isOpen)}\n        style={{\n          padding: '6px 12px',\n          borderRadius: '4px',\n          border: '1px solid #ccc',\n          background: 'white',\n          cursor: 'pointer',\n          display: 'flex',\n          justifyContent: 'space-between',\n          alignItems: 'center',\n          minHeight: '20px'\n        }}\n      >\n        <span>\n          {selectedOption ? (\n            <span>\n              {selectedOption.name} ({selectedOption.cost} XP)\n              {selectedOption.restrictions && (\n                <span> - {parseXWingText(selectedOption.restrictions)}</span>\n              )}\n              {selectedOption.invalidText && (\n                <span style={{ color: '#6c757d', fontStyle: 'italic' }}> {selectedOption.invalidText}</span>\n              )}\n            </span>\n          ) : (\n            placeholder\n          )}\n        </span>\n        <span style={{ color: '#666', fontSize: '12px' }}>{isOpen ? '^' : 'v'}</span>\n      </div>\n      \n      {isOpen && (\n        <div\n          style={{\n            position: 'absolute',\n            top: '100%',\n            left: 0,\n            right: 0,\n            background: 'white',\n            border: '1px solid #ccc',\n            borderTop: 'none',\n            borderRadius: '0 0 4px 4px',\n            maxHeight: '200px',\n            overflowY: 'auto',\n            zIndex: 1000,\n            boxShadow: '0 2px 8px rgba(0,0,0,0.1)'\n          }}\n        >\n          <div\n            onClick={() => handleSelect({ value: '', name: '', cost: 0 })}\n            style={{\n              padding: '8px 12px',\n              cursor: 'pointer',\n              borderBottom: '1px solid #eee'\n            }}\n            onMouseEnter={(e) => e.target.style.background = '#f0f0f0'}\n            onMouseLeave={(e) => e.target.style.background = 'white'}\n          >\n            {placeholder}\n          </div>\n          {options.map((option, index) => (\n            <div\n              key={index}\n              onClick={() => handleSelect(option)}\n              style={{\n                padding: '8px 12px',\n                cursor: 'pointer',\n                borderBottom: index < options.length - 1 ? '1px solid #eee' : 'none',\n                color: option.isInvalid ? '#6c757d' : 'inherit',\n                fontStyle: option.isInvalid ? 'italic' : 'normal'\n              }}\n              onMouseEnter={(e) => e.target.style.background = '#f0f0f0'}\n              onMouseLeave={(e) => e.target.style.background = 'white'}\n            >\n              <span>{option.name} ({option.cost} XP)</span>\n              {option.restrictions && (\n                <span> - {parseXWingText(option.restrictions)}</span>\n              )}\n              {option.invalidText && (\n                <span style={{ color: '#6c757d' }}> {option.invalidText}</span>\n              )}\n            </div>\n          ))}\n        </div>\n      )}\n    </div>\n  );\n};
...
// Text-only version for dropdown options (no JSX)
const parseXWingTextToString = (text) => {
  if (!text) return text;
  
  return text.replace(/\[([^\]]+)\]/g, (match, code) => {
    if (code.startsWith('r:')) {
      // Red colored symbol: just return the symbol without color for text
      return code.substring(2);
    } else {
      // Regular symbol: just return the symbol
      return code;
    }
  });
};
            ...
            <div key={index} style={{ marginBottom: '15px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <CustomDropdown
                  ...
                  options={availableUpgrades.map(upgrade => {
                    const isInvalid = upgrade.minimum_in && characterInitiative < upgrade.minimum_in;
                    return {
                      value: upgrade.name,
                      name: upgrade.name,
                      cost: Number(upgrade.cpp_cost),
                      restrictions: upgrade.restrictions,
                      isInvalid,
                      invalidText: isInvalid ? `[Requires Init ${upgrade.minimum_in}]` : null
                    };
                  })}
                  onChange={(value) => handleUpgradeSelection(index, value)}
                  style={{ flex: 1 }}
                />
              </div>
                          ...
                          <div key={index} style={{ 
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                              ...
                              <CustomDropdown
                                ...
                                onChange={(value) => handleRankUpgradeSelection(slotIndex, value)}
                                style={{ flex: 1 }}
                              />
                            </div>
